...
export const TransactionCard: React.FC<Props> = ({ tx, onDeleted }) => {
//export const TransactionCard: React.FC<{ tx: Transaction }> = ({ tx}) => {
  ...
  const Delete = async (e: React.MouseEvent<HTMLButtonElement>) => {
    e.stopPropagation();
     try {
  ...
  return (
    ...
    {/* 🔹 UPDATED: show Edit + Delete OR Yes/No confirm */}
      {!confirming ? (
